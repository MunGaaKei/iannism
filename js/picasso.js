;(function(win, doc, undefined){
    ...
    doc.addEventListener('DOMContentLoaded', function(){
        ...
        if (/windows|win32/i.test(navigator.userAgent)) {
            doc.documentElement.classList.add('os-windows');
        }
        ...
        photograhp.addEventListener('open', function(){
            initPhotos( this.querySelectorAll('.photos img') );
        }, { once: true });
        ...
        function initPhotos( imgs ){
            ...
            for(; i < l; i++){
                img = imgs[i];
                ispc && (img.style.cssText = 'transform: translate(-50%,-50%) rotate('+ (i * 12 - 16) + 'deg)');
